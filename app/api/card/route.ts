...
export async function GET(request: NextRequest) {
  try {
    ...
    const svg = generateEnhancedStreakCard(data, theme)
...
function generateEnhancedStreakCard(data: any, theme: any): string {
  ...
  <text x="10" y="190" fill="${theme.textColor}" font-size="8" opacity="0.5" font-family="Arial, sans-serif">
    Updated: ${new Date().toLocaleDateString()} • Avatar: ${avatarUrl ? "Loaded" : "Fallback"}
  </text>
</svg>`
}
